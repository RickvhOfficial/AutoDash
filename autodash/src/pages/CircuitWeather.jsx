    ...
    <div className="pointer-events-none rounded-none bg-transparent p-0">
      {payload.map((entry) => (
        <p
          ...
          className="mb-1 inline-block rounded-md border border-slate-500/60 bg-slate-950/90 px-2 py-0.5 text-base font-semibold shadow-lg"
        >
...
function MetricBlock({
  ...
  return (
    <article className="grid h-full min-h-[10.5rem] gap-3 rounded-xl border border-slate-700 bg-slate-900/75 p-4 sm:grid-cols-[0.95fr_1.35fr] sm:items-center">
      ...
      <div className="h-28 min-w-0 overflow-hidden rounded-lg border border-slate-700 bg-slate-950/80 p-1 md:h-32">
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'area' ? (
              <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                <CartesianGrid stroke="#334155" strokeDasharray="3 3" />
                <XAxis dataKey="day" tick={{ fill: '#94a3b8', fontSize: 10 }} />
                <YAxis tick={{ fill: '#94a3b8', fontSize: 10 }} width={26} />
                <Tooltip
                  trigger="hover"
...
export default function CircuitWeather() {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col bg-slate-950 text-slate-100">
      <div className="relative z-10 w-full shrink-0 border-b border-slate-800" style={{ height: WEATHER_HERO_HEIGHT_PX, maxHeight: WEATHER_HERO_HEIGHT_PX }}>
        <img
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 px-4 pb-4 sm:px-6 md:px-8 lg:px-10">
          <div className="relative ml-0 inline-flex w-full max-w-[30rem] flex-col rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-3 backdrop-blur-sm sm:ml-2 md:ml-0 md:px-4 md:py-3 lg:ml-[16rem]">
            <div>
                ...
                <div className="relative z-[120] mt-2 block max-w-md" ref={circuitMenuRef}>
                  ...
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-lg border border-slate-600 bg-slate-800 px-3 py-1.5 text-left text-sm text-slate-100 outline-none ring-0 transition hover:border-slate-500 focus:border-red-500 focus:ring-2 focus:ring-red-500/30"
                  ...
                  >
                    ...
                    <span aria-hidden className="ml-2 text-xs text-slate-300">
                      {isCircuitMenuOpen ? '▲' : '▼'}
                    </span>
                  </button>

                  {isCircuitMenuOpen && (
                    <div className="absolute left-0 right-0 z-[999] mt-1 overflow-hidden rounded-lg border border-slate-600 bg-slate-800 shadow-2xl">
        ...
        <p className="pointer-events-none absolute bottom-2 right-4 z-20 text-right text-xs text-slate-300 sm:right-6 sm:text-sm md:right-10">
          Bijgewerkt:{' '}
          ...
          {!loading && !error && weather && (
            <div className="flex flex-col gap-8">
              <div className="flex flex-col items-stretch gap-4 xl:flex-row xl:items-stretch">
                <article className="relative w-full overflow-hidden rounded-2xl border border-slate-700/90 bg-slate-900/60 shadow-lg xl:w-[27%] xl:min-w-[16rem] xl:max-w-[19rem]">
                  <div
                  ...
                  <div className="relative z-10 flex h-full flex-col justify-center p-4 sm:min-h-[10rem]">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-slate-300">{currentLabel}</p>
                      </div>
                    </div>
                    <dl className="relative left-0 mt-3 grid grid-cols-3 gap-2 text-center text-xs sm:-left-2.5">
                      <div className="flex flex-col items-center gap-0.5">
                        <dt className="leading-none text-xs text-slate-400">Wind</dt>
                        <dd className="leading-none font-semibold text-white">
                      ...
                      <div className="flex flex-col items-center gap-0.5">
                        <dt className="leading-none text-xs text-slate-400">Neerslag</dt>
                        <dd className="leading-none font-semibold text-white">
                      ...
                      <div className="flex flex-col items-center gap-0.5">
                        <dt className="leading-none text-xs text-slate-400">Luchtvochtigheid</dt>
                        <dd className="leading-none font-semibold text-white">
